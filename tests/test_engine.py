...
def _hand_prefix():
    ...
    pivot low 8 @2 -- the true origin -- pivot high 15 @3, pivot low 9.5
    @4) followed by an impulsive rally (idx5-9) that carves a bullish FVG
    at gap=[20, 30] (c1=idx5 high=20, c3=idx7 low=30), confirmed by
    idx8-9, and finally a new swing high of 50 at idx10.

    With extreme=50 (idx10) and the true origin prior=8 (idx2),
    leg_range=42 and the fib band (ratios 0.382/0.295/0.21) works out to
    [16.82, 24.044] -- so the FVG/fib intersection ("the zone") is
    [20, 24.044], not the full FVG [20, 30].
    """
    return [
        (9.5, 10, 9, 9.8),      # 0 filler
        (11.2, 12, 11, 11.8),   # 1 pivot high 12
        (8.6, 9, 8, 8.4),       # 2 pivot low 8 (true origin)
        (13.5, 15, 13, 14.5),   # 3 pivot high 15
        (9.8, 10, 9.5, 9.6),    # 4 pivot low 9.5 (minor, must NOT be selected as prior)
...
ZONE_LOW = 20
ZONE_HIGH = 24.044  # intersection of FVG [20, 30] and the fib band [16.82, 24.044]


def _decline(start_price, end_price, start_idx, count):
    """Small-bodied candles gradually declining (or climbing, if
    end_price > start_price) in a straight line -- keeps the regression
    over this stretch gentle, and never produces an accidental
    >70%-body candle.
    """
    rows = []
...
def build_candles(tail_rows):
    # Declines from just under the swing high down to ~22, comfortably
    # inside the [20, 24.044] intersection zone, so the tail candle
    # determines the final status from there.
    rows = _hand_prefix() + _decline(46, 22, 11, 14) + tail_rows
    return [
...
class TestEnginePipeline(unittest.TestCase):
    def test_in_range(self):
        candles = build_candles([(23, 24, 21, 22)])
        ...
        self.assertEqual((result.signal.zone_low, result.signal.zone_high), (ZONE_LOW, ZONE_HIGH))

    def test_market_breakout(self):
        candles = build_candles([(22, 30, 21, 29)])
        result = engine.analyze(candles, "TESTUSDT", "5m", "LONG", CFG)
        self.assertIsNotNone(result.signal)
        self.assertEqual(result.signal.status, "MARKET")
        self.assertEqual(result.signal.entry, 29)
        self.assertEqual(result.signal.stop_loss, ZONE_LOW)
    ...
    def test_steep_retracement_is_rejected_with_angle_in_reason(self):
        # The regression is fit over the WHOLE leg (true origin through
        # to the first zone touch), so it's deliberately robust to a
        # single noisy candle -- reaching an unambiguously "steep"
        # regression slope needs a synthetic, exaggerated crash (not
        # meant to be a realistic price, just a reliable way to drive the
        # fitted slope well past the threshold). See
        # tests/test_slope.py::TestRegressionAngle for realistic-shaped
        # shallow/steep cases against compute_regression_angle_degrees
        # directly.
        rows = _hand_prefix() + [(49, 50, -1000, -1000)]
        candles = [
            Candle(index=i, timestamp=i * 60, open=o, high=h, low=l, close=c, volume=0.0)
            for i, (o, h, l, c) in enumerate(rows)
        ]
    ...
    def test_price_in_fvg_but_outside_fib_zone_is_not_a_signal(self):
        # Price wicks into the part of the FVG [20, 30] that falls OUTSIDE
        # the fib band -- the decline here bottoms out around 26-27, well
        # within the raw FVG but never reaching down to the
        # intersection's 24.044 ceiling -- must not count as reaching the
        # zone at all. Uses its own decline (not build_candles', which
        # dips all the way into the real intersection) so no candle ever
        # touches [ZONE_LOW, ZONE_HIGH].
        rows = _hand_prefix() + _decline(46, 27, 11, 14) + [(28, 29, 26.5, 27)]
        candles = [
            Candle(index=i, timestamp=i * 60, open=o, high=h, low=l, close=c, volume=0.0)
            for i, (o, h, l, c) in enumerate(rows)
        ]
    ...
    def test_price_in_the_actual_intersection_proceeds_normally(self):
        # Same FVG, but this time price actually reaches into
        # [ZONE_LOW, ZONE_HIGH] -- normal signal logic must proceed.
        candles = build_candles([(23, 24, 21, 22)])
        result = engine.analyze(candles, "TESTUSDT", "5m", "LONG", CFG)
        self.assertIsNotNone(result.signal)
        self.assertEqual(result.signal.status, "IN_RANGE")
